...
// ======================================================
// ✅ NUEVO: SUBIR AVATAR (PERSISTENTE) A CLOUDINARY
...
app.post("/upload-avatar", upload.single("file"), async (req, res) => {
  try {
    ...
    if (!allowed.includes(req.file.mimetype)) {
      return res.status(400).json({ error: "Formato no permitido (JPG/PNG/WebP)" });
    }
...
playlist.isPublic = isPublicBool;

// 🔥 AÑADE ESTO SIEMPRE
if (!isPublicBool) {
  playlist.shareToken = playlist.shareToken || crypto.randomBytes(16).toString("hex");
}
...
// ✅ CORRECCIÓN: catch-all SOLO PARA GET y con RegExp (Express 5 / Render compatible)
app.get(/.*/, (req, res) => {
